...
export function BillingPage() {
  ...
  async function upgrade(plan: 'basic' | 'pro') {
    setLoading(true)
    const { url } = await apiFetchAuth<{ url: string }>(`/api/billing/checkout?plan=${plan}`)
  ...
  const planLabel = t(`billing.plan${planKey.charAt(0).toUpperCase() + planKey.slice(1)}`)
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="p-6 bg-[var(--color-surface)] rounded-2xl mb-6">
        <p className="text-sm text-[var(--color-text-primary)]/50 mb-1">{t('billing.currentPlan')}</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          {tenant?.plan === 'free' && (
            <button onClick={() => upgrade('basic')} disabled={loading}
              ...
              <p className="text-2xl font-display mb-2">{t('billing.basicPrice')}</p>
              <p className="text-sm text-[var(--color-text-primary)]/50">{t('billing.basicFeatures')}</p>
            </button>
          )}
          <button onClick={() => upgrade('pro')} disabled={loading}
            className="p-6 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-gold)]/30 hover:border-[var(--color-gold)] transition-colors text-left">
            <p className="font-semibold mb-1">{t('billing.planPro')}</p>
            <p className="text-2xl font-display mb-2">{t('billing.proPrice')}</p>
            <p className="text-sm text-[var(--color-text-primary)]/50">{t('billing.proFeatures')}</p>
          </button>
        </div>
      )}
